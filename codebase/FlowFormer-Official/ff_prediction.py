...
SAVETXTPATH = os.path.join(OUTPUT_DIR, 'ff_time.txt')
...
# calculate the error of the overall optical flow method after passing through the memristor
...
def viz(flo, imgname):  
    ...
    # imgpil.save(imgname)
    imgpil.save(imgname, 'PNG', quality=100, dpi=(300, 300))  # 使用PNG格式，最高质量，300DPI
...
# use opticalFlow3D to calculate the optical flow
def opticalFlow3D(model, memimg1, memimg2, rgbimg1, rgbimg2, pixel_width, pixel_height):
    ...
    # there are regions that need to be processed, choose the processing method according to FLAG
    if FLAG == 1:
...
def task_results(prev_frame, next_frame, flow, num_labels, regions_info, EST_FLAG, MERGE_FLAG):
    ...
    MERGE_FLAG: when FLAG=1 is effective, True represents merged processing, False represents separate processing
    """
    start_time = time.time()
    ...
    if num_labels > 1:
        if EST_FLAG == 1:
            if MERGE_FLAG:
                ...
                # the corresponding prediction task function
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for idx in range(len(rgbimages)-2):
            ...
            # the number of pixels in the original image corresponding to one pixel in the image after processing by the memristor
            pixel_width = MEMSIZE
            ...
            if FLAG == 1:
                ...
                new_frame_rgb = task_results(
                    ...
                    flow, 
                    num_labels,
                    (x_start, y_start, x_end, y_end),  # directly pass the region boundary coordinates
                    EST_FLAG=FLAG,
                    MERGE_FLAG=False  # this parameter is invalid when FLAG=2
                )
            ...
            ############### original flow former ##############
            start_time = time.time()
            flow1 = runflowformer(model, rgbimg1, rgbimg2)
            end_time = time.time()
            flow1 = flow1.astype(np.float32)
            original_opticalflow_times.append(end_time - start_time)

            # save the original optical flow
            ...
            if cnt != 0:
                ...
                ssim2 += raft_ssim
                # with open(SAVETXTPATH, 'a', encoding='utf-8') as file:
                #     file.write(f'using original raft predict {filename3} ssim is {memraft_ssim}\n')
            ...
            if cnt < len(rgbimages) - 1:
                ...
                # if it is the last frame, fill NA
        ...
        print(f'mem flowformer average ssim is {ssim1/(cnt-1)}, original flowformer average ssim is {ssim2/(cnt-1)}\n')
        print("Processing complete.")
        with open(SAVETXTPATH, 'a', encoding='utf-8') as file:
            file.write(f'mem flowformer average ssim is {ssim1/(cnt-1)}, original flowformer average ssim is {ssim2/(cnt-1)}\n')