...
def opticalFlow3D(model, memimg1, memimg2, rgbimg1, rgbimg2, pixel_width, pixel_height):
    ...
    # if there are regions to process, choose the processing method based on FLAG
    if FLAG == 1:
...
def task_results(prev_frame, next_frame, flow, num_labels, regions_info, EST_FLAG, MERGE_FLAG):
    ...
    if num_labels > 1:
        if EST_FLAG == 1:
            ...
            # directly use the merged region to process
...
def process_flow_region(mag, ang):
    """
    process the optical flow data of a specific region and generate a motion segmentation mask
    """
    # create an HSV image of the region
...
def calculate_seg(flow, hsv):
    ...
    # for each pixel, check if the flow magnitude is greater than the threshold, and mark the pixel as a motion region
    motion_mask[mag > threshold] = 255

    # morphological operation on the motion region to fill holes or remove noise
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (10, 10))
    ...
    # convert the binary image of the motion region to a grayscale image
    motion_gray = cv2.cvtColor(motion_mask, cv2.COLOR_BGR2GRAY)

    # threshold the grayscale image to get the final binary image
